...
namespace BeanBrowser.Connector
{
	...
	public class Bean
	{
		...
		{
			this.watcher = PnpObject.CreateWatcher(PnpObjectType.DeviceContainer, new[] { "System.Devices.Connected" }, String.Empty);
			this.watcher.Updated += DeviceConnectionUpdated;
			this.watcher.Start();
		}
		...
		/// Register to be notified when a connection is established to the Bluetooth device
		/// </summary>
		private void StopDeviceConnectionWatcher()
		{
		...
		public async Task Subscribe(String deviceId)
		{
			...
			try
			{
				...
				this.currentService = await GattDeviceService.FromIdAsync(deviceId);
				...
				IReadOnlyList<GattCharacteristic> chars = this.currentService.GetCharacteristics(BeanConstants.BeanSerialCharUuid);
				...
				this.deviceContainerId = "{" + di.Properties["System.Devices.ContainerId"] + "}";
				StartDeviceConnectionWatcher();
				this.currentCharacteristic = c;
		...
		public void Unsubscribe()
		{
			...
			this.currentCharacteristic.ValueChanged -= OnValueChanged;
